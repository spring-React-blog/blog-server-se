...
public class BoardController {
   ...
   /* @GetMapping("/boards/category/{parents}/{child}")
    ...
    @PutMapping("/boards")
    public ResponseEntity<Long> update(@RequestBody @Valid BoardRequest request, @AuthenticationPrincipal CustomUserDetails userDetails){
        Board board = request.toEntity();
        boardService.updateBoard(board, board.getId(), userDetails.getUsername());

        return new ResponseEntity(HttpStatus.OK);
    }

    @DeleteMapping("/boards/{boardId}")
    public ResponseEntity<BoardResponse> delete(@PathVariable Long boardId, @AuthenticationPrincipal CustomUserDetails userDetails){
